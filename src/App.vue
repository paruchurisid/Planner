<template>
  ...
  <div v-show="compatible" id="app-container" class="app-container" :class="{ 'dark-theme': darkTheme }" :style="backgroundStyle">
    <div v-if="backgroundImage" class="background-image" :style="{ backgroundImage: `url(${backgroundImage})` }"></div>
    <div class="hidden-mobile app-body" :style="{ zoom: `${zoom}%` }">
      <splash-screen ref="splash"></splash-screen>
      <side-bar 
        @change-date="setSelectedDate"
        @open-background-settings="openBackgroundSettings"
      ></side-bar>
  ...
  <background-settings-modal 
    ...
    @background-selected="setBackground"
  />
</template>
...
import BackgroundSettingsModal from "./components/BackgroundSettingsModal.vue";
  ...
  computed: {
    backgroundStyle() {
      return {
        position: 'relative',
        minHeight: '100vh',
        backgroundColor: this.backgroundImage ? 'rgba(255, 255, 255, 0.9)' : 'var(--body-bg)'
      };
    },
